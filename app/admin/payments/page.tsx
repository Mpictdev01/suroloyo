    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Review bukti transfer dan konfirmasi pembayaran booking</p>
        </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 px-6 py-3 rounded-2xl flex items-center gap-3 shadow-lg shadow-amber-500/5">
          ...
          <div>
            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-wider">Menunggu Verifikasi</p>
            <p className="text-white text-xl font-black leading-none mt-0.5">{payments?.length || 0}</p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1,2,3].map(i => (
                <div key={i} className="bg-surface-dark border border-border-dark rounded-2xl h-80 animate-pulse" />
            ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-surface-dark border border-border-dark rounded-3xl text-center">
            <span className="material-symbols-outlined text-gray-700 text-8xl mb-4">check_circle</span>
            <h3 className="text-xl font-bold text-white mb-2">Semua Beres!</h3>
            <p className="text-gray-500">Tidak ada pembayaran yang butuh verifikasi saat ini.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col gap-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-white font-bold text-lg">{p.users?.name || 'Unknown User'}</h3>
                    <p className="text-gray-500 text-xs">{p.users?.email || '-'}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-primary font-mono font-bold text-sm">#{p.id}</p>
                ...
                <div className="bg-background-dark/50 rounded-xl p-3 border border-border-dark/50 flex items-center justify-between text-xs">
                    <div className="flex flex-col">
                        <span className="text-gray-500 font-bold uppercase tracking-wider text-[9px]">Tgl Mendaki</span>
                        <span className="text-gray-300 font-bold mt-0.5">{new Date(p.booking_date).toLocaleDateString('id-ID', { dateStyle: 'medium' })}</span>
                    </div>
                    <div className="flex flex-col text-right">
                        <span className="text-gray-500 font-bold uppercase tracking-wider text-[9px]">Waktu Bayar</span>
                        <span className="text-gray-300 font-bold mt-0.5">{new Date(p.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB</span>
                    </div>
                </div>
